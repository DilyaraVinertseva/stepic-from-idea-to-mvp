...
def open_data(path="https://github.com/DilyaraVinertseva/stepic-from-idea-to-mvp/blob/d2a73a7c5e09086bb7764a68d0b6c169e36061a5/clients.csv"):
    df = pd.read_csv(path)
...
def preprocess_data(df: pd.DataFrame, test=True):
    ...
    for col in cols:
      ...
      | (df[col] == 3.0) | (df[col] == 4.0) | (df[col] == 5.0)]
        df.loc[df[col] == 0.0, col] = float(df[col].mode())
...
def fit_and_save_model(X_df, y_df, path="model.pickle"):
    ...
    score = roc_auc_score(test_prediction, y_df)
