...
'''
    ...
    age = st.sidebar.slider("Возраст", min_value=0, max_value=100, value=20,
                            step=1)
    customer_type = st.selectbox('Тип_лояльности', ('Лояльный', "Нелояльный"))
    type_of_travel = st.selectbox("Тип поездки", ("Рабочая", "Частная"))
    servclass = st.selectbox("Класс обслуживания", ("Business", "Eco", "Eco Plus"))
    flight_distance = st.slidebar.slider("Дальность перелета", 
                        min_value=0, max_value=9320, value=500, step=1)
    departure_delay_in_minutes = st.slidebar.slider("Задержка отправления", 
                        min_value=0, max_value=25000, value=0, step=1)
    arrival_delay_in_minutes = st.slidebar.slider("Задержка прибытия", 
                        min_value=0, max_value=15000, value=0, step=1)
    inflight_wifi_service = st.slidebar.slider("Оценка клиентом интернета на борту",
                        min_value=1, max_value=5, value=1, step=1)
    dep_arvl_time_convenient = st.slidebar.slider("Оценка клиентом удобства времени прилета и вылета", 
                        min_value=1, max_value=5, value=1, step=1)
    ease_of_online_booking = st.slidebar.slider("Оценка клиентом удобства онлайн-бронирования",
                         min_value=1, max_value=5, value=1, step=1)
    gate_location = st.slidebar.slider("Оценка клиентом расположения выхода на посадку в аэропорту", 
                        min_value=1, max_value=5, value=1, step=1)
    food_and_drink = st.slidebar.slider("Оценка клиентом еды и напитков на борту",
                        min_value=1, max_value=5, value=1, step=1)
    online_boarding = st.slidebar.slider("Оценка клиентом выбора места в самолете", 
                        min_value=1, max_value=5, value=1, step=1)
    seat_comfort = st.slidebar.slider("Оценка клиентом удобства сиденья", 
                        min_value=1, max_value=5, value=1, step=1)
    inflight_entertainment = st.slidebar.slider("Оценка клиентом развлечений на борту", 
                        min_value=1, max_value=5, value=1, step=1)
    on_board_service = st.slidebar.slider("Оценка клиентом обслуживания на борту", 
                        min_value=1, max_value=5, value=1, step=1)
    leg_room_service = st.slidebar.slider("Оценка клиентом места в ногах на борту", 
                        min_value=1, max_value=5, value=1, step=1)
    baggage_handling = st.slidebar.slider("Оценка клиентом обращения с багажом", 
                        min_value=1, max_value=5, value=1, step=1)
    checkin_service = st.slidebar.slider("Оценка клиентом регистрации на рейс", 
                        min_value=1, max_value=5, value=1, step=1)
    inflight_service = st.slidebar.slider("Оценка клиентом обслуживания на борту", 
                        min_value=1, max_value=5, value=1, step=1)
    cleanliness = st.slidebar.slider("Оценка клиентом чистоты на борту", 
                        min_value=1, max_value=5, value=1, step=1)
